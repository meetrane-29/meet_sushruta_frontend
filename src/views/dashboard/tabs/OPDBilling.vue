      ...
      <div v-if="activeBillingTab === 'generate'" class="space-y-4">
        ...
        <div v-else class="grid grid-cols-2 gap-4">
          <div>
            ...
            />
            <p class="text-xs text-gray-400 mt-1">Or select a patient from 'Patients Today' above</p>
          </div>
          <div>
...
const generateReceipt = async () => {
  try {
    ...
    if (!patientId) {
      errorMessage.value = 'Patient ID required. Please select a patient from Patients Today above.'
      return
    }
