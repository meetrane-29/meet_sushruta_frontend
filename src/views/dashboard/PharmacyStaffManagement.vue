...
// Fetch all pharmacy staff
const fetchPharmacyStaff = async () => {
  loading.value = true
  error.value = ''

  try {
    // Call API to get pharmacy staff
    ...
    error.value = err.response?.data?.message || err.response?.data?.error || err.message || 'Failed to load pharmacy staff. Please try again.'
    console.error('Error fetching pharmacy staff:', {
